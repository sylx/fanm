...
- 絵は描画命令、`gfx.drawImage` に渡す 1 画素 1 バイトの配列、スプライトで作る。スクロールで流す平面の、`gfx` が届かない行（212〜255行目）だけは、型の手引きに従ってビデオメモリへ直に書いてよい。音は MML（`compile` と `ctx.bgm`）か PSG / OPLL の直接操作で作る。
