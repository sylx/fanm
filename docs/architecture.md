...
| キャラクタ画面 | `templates/tiles/` | SCREEN 1 / 2 / 4 の PCG で組む。画面は `NameBuffer` で毎フレーム作り直し、動く絵は文字の定義を書き換えて作る |
...
- キャラクタ画面にはビットマップのモードも描き足しも無いので、共通の縛りのうち「画面モード」と「動きの出どころ」は引かず、型の札で代える（`plan/variations.ts` の `SKIP_COMMON`）。
- 主人公は多色スプライト（`sprites.setMulticolorPattern` / `setMulticolor`）で描く。ゲーム、横スクロール、縦シューティング、キャラクタ画面の実例がそうしているので、生成される作品も一色一行に戻らない。
...
| 部分 | できていること | まだないもの |
| --- | --- | --- |
| バッチ | `fanm make`：企画 → 生成 → 検査 → 修正（最大2回）→ 採用/不採用 を一つのジョブとして回す。ジョブ状態と予算台帳を保存し、途中から再開できる。DeepSeek・Claude・OpenAI の接続、APIキーなしで試す偽のAI（`--fake`）。頼む相手はジョブごとに札束（`providers/deck.ts`）から引き、作り手の性格とペンネームも引く（`persona/`）。検査は静的検査・書き写しの検査・型検査・隔離実行・撮影・画面の数値判定。企画には型のほかに縛り（`plan/variations.ts`）を引いて渡し、実例はデータを抜いて渡す。`fanm run`：残予算と実費から頻度を決めて回し続け、頃合いを見て公開し、直らない問題だけ知らせる。設定は30秒ごとに読み直すので、予算・頻度・札束は動かしたまま変えられる。`fanm status` と Dockerfile／docker-compose.yaml（[docs/operate.md](operate.md)） | 不採用作の掃除、ブラウザでの確認、Coolify への初回デプロイ |
